...
NOTE: If you test the app using npm run start (vite preview on port 3002+), searches may fail with a JSON parse error. This is expected because the backend/API proxy is not active in preview mode.
API works as expected in:
Development (npm run dev at localhost:3000)
Production (https://newsexplorer.flazzard.com)
